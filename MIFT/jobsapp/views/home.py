...
class HomeView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # context['trendings']=self.model.objects.latest('created_at')[:20]

        context['trendings'] = self.model.objects.filter(created_at__month=timezone.now().month)[:20]
        return context
...
class SearchView(ListView):
    ...
    def get_queryset(self):
        ...
        return self.model.objects.filter(location__contains=self.request.GET['location'],
                                         last_date__contains=self.request.GET['last_date'])
...
class JobUpdateView(UpdateView):
    ...
    fields = ['title','description','location','type','last_date','company_name','company_description','website','phone_number']
    pk_url_kwarg = 'id'
    ...
    def form_valid(self, form):
        ...
        post.save()
        print(post.phone_number)
        return redirect(reverse_lazy("jobs:ngo-dashboard"))
